...
class MainMenuViewController: CustomViewController {
    ...
    private func checkPlayerName(completion: @escaping (Bool) -> Void) {
        if let data = userDefaults.value(forKey: .userSettings) as? Data {
            do {
                ...
                if user.playerName.isEmpty {
                    self.showAlertWithTextField(titleAlert: "Player name",
                                                message: "Please enter the player's name to save the results",
                                                style: .alert) { playerName in
            ...
            checkPlayerName { nameExist in
                if nameExist {
                    UIView.animate(withDuration: 0.3, delay: 0.4, options: .curveEaseIn, animations: {
                        ...
                        self.showViewController(showingViewController: GameViewController(),
